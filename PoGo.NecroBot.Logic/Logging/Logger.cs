...
namespace Cv3PKM.Bot.Logic.Logging
{
    public static class Logger
    {
                ...
                using (
                    var log =
                        File.AppendText(Path.Combine(_path,
                            $"Cv3PKM-{DateTime.Today.ToString("yyyy-MM-dd")}-{DateTime.Now.ToString("HH")}.txt"))
                    )
                {
        ...
        public static void SetLogger(ILogger logger, string subPath = "", bool isGui = false)
        {
            ...
            if (!_isGui)
            {
                ...
                Log($"Initializing Cv3PKM logger at time {DateTime.Now}...");
            }
        }
